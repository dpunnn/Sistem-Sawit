...
import { gradeImage } from '@/lib/api'
...
export default function GatePage() {
  const [result, setResult] = useState<GradingResult>(demoGrading)
  ...
  async function onPick(file: File) {
    ...
    setDecision(null)
    setScenario(null)
    const { data, source } = await gradeImage(file)
    setResult(data)
    setSource(source)
    setStatus('done')
  }
    ...
    <main className="mx-auto max-w-6xl px-5 py-8">
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_1fr]">
        ...
        <Card>
          ...
          />
          <CardBody>
            <div className={status === 'scanning' ? 'opacity-60 transition' : 'transition'}>
              <DetectionOverlay
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <Button onClick={() => inputRef.current?.click()}>Pindai ulang</Button>
            </div>
            ...
            <div className="mt-4 border-t border-line pt-3">
              ...
              <p className="mt-2 text-[12px] leading-relaxed text-ink-soft">
                Bandingkan keduanya: potensi minyak turun, dan selangnya ikut
                melebar. Muatan yang komposisinya timpang memang lebih sulit
                ditaksir dari lapisan permukaan — sistem mengakuinya, bukan
                menutupinya.
              </p>
            </div>

            <p className="mt-3 text-[12px] text-ink-soft">
              Model {result.model_version} · {result.detections.length} tandan terdeteksi
              {status === 'scanning' && ' · memproses…'}
            </p>
          </CardBody>
        </Card>
            ...
            <CardBody>
              ...
              {decision && (
                <p className="mt-3 text-[13px] leading-relaxed text-ink-soft">
                  {decision === 'agree'
                    ? 'Hasil disetujui. Sertifikat sortasi diterbitkan dan muatan di-tag ke batch.'
                    : 'Koreksi grader disimpan sebagai data latih untuk kalibrasi ulang mingguan.'}
                </p>
              )}
